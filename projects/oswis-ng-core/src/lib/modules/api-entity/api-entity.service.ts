...
export class ApiEntityService implements ApiEntityInterfaceService {
    ...
    return this.http.put<Type[]>(this.getApiUrl() + '/' + updatedEntity.id, updatedEntity)
      .pipe(
        tap(() => {
          ...
          const message = this.getEntityName() + ' ' + updatedEntity.id + ' ' +
            ' byl' + this.getA() + ' úspěšně upraven' + this.getA() + '.';
          this.callRefreshCallbacks();
          if (callback) {
            callback();
          }
  ...
  delete(entity: Type, callback?: any): Observable<Type> {
    if (entity && entity.id > 0) {
      return this.deleteById(entity.id, callback);
    }
    return new Observable<Type>();
  }
